...
/**
 *  用于去除 foobar 中对于补丁制作无用的 section；
 */
		      ...
		      unsigned char *text)
{
	...
	if (GELF_ST_BIND(sym->st_info) == STB_LOCAL &&
		...
		rel->r_info = GELF_R_INFO(section_symn, GELF_R_TYPE(rel->r_info));
