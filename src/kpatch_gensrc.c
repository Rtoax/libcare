...
{
    //print_hello, @function -> print_hello.kpatch
    warn_log("rename add: %s->%s\n", src->s, dst->s);
	struct rename *r, *r2;
...
static void cblock_var_check_content(struct cblock *b0)
{
    ...
    debug_log("check block: %s vs %s\n", b0->name.s, b1?b1->name.s:"null");
...
/**
 *  <print_hello, @function> <print_hello, @function> <.kpatch>
 */
...
{
    debug_log("new name: <%s> <%s> <%s>\n", b->name.s, basename->s, suffix);
...
static void analyze_var_cblocks_matches(struct kp_file *f0, struct kp_file *f1)
{
	...
	for (b0 = cblock_skip(cblock_first(f0), CBLOCK_VAR); b0; b0 = cblock_skip(cblock_next(b0), CBLOCK_VAR)) {
        ...
        if (!b0->pair) {
            ...
            debug_log("Variable %.*s was removed or changed? Patch requires adoptation\n", b0->name.l, b0->name.s);
        }
	}
}
